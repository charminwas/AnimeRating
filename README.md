...
## **备注**
- 如果想重新爬取数据：虽然爬虫设置了延迟、爬取量限制等，仍建议将任务拆分为多个部分，少量多次爬取
- 如果想更改数据处理与画图部分：数据过滤条件（保留季度总数为多少、评分人数为多少的动漫系列）可以在`4_filter.py`中修改；可视化图表样式也可以在`6_draw_figure.py`中修改
- 以上动漫数据提取于2026/03/05
- 代码与README的写作有大语言模型的辅助
